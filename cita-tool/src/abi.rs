...
pub fn parse_tokens(params: &[(ParamType, &str)], lenient: bool) -> Result<Vec<Token>, ToolError> {
    ...
    for (param, value) in params {
        ...
        res.push(token.map_err(|e| ToolError::Abi(e.to_string()))?);
    }
    Ok(res)
}
...
pub fn decode_input(
    ...
    let function = contract
        ...
        .map_err(|e| ToolError::Abi(format!("{e}")))?;
    let tokens = function
        .decode_output(&hex_decode(data).map_err(|e| ToolError::Abi(format!("{e}")))?)
        .map_err(|e| ToolError::Abi(format!("{e}")))?;
    ...
    let result = types
        .zip(tokens.iter())
        .map(|(ty, to)| {
            let nt_to = NewToken::from(to.clone());
            if to.type_check(&ParamType::Bool) || format!("{ty}") == "bool[]" {
                format!("{{\"{ty}\": {nt_to}}}")
            } else {
                let to_str = format!("{nt_to}");
                let escaped_str = to_str.escape_default();
                format!("{{\"{ty}\": \"{escaped_str}\"}}")
            }
        })
