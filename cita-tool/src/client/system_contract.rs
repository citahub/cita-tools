    ...
    E: Fail,
{
    ...
    fn query_accounts_num(&self, address: &str, height: Option<&str>) -> Result<R, E> {
        self.contract_call_to_address("queryAccountsNum", &[], address, height)
    }
    ...
    E: Fail,
{
    ...
    fn query_account_groups(&self, account: &str, height: Option<&str>) -> Result<R, E> {
        ...
        self.contract_call("queryAccountGroups", &values, None, height)
    }
}
    ...
    E: Fail,
{
    ...
    fn query_all_permissions(&self, height: Option<&str>) -> Result<R, E> {
        self.contract_call("queryAllPermissions", &[], None, height)
    }
    ...
    fn query_all_permissions_number(&self, height: Option<&str>) -> Result<R, E> {
        self.contract_call("queryAllPermissionsNumber", &[], None, height)
    }
    ...
    /// Query all accounts number
    ///
    /// return All the accounts number
    fn query_all_accounts_number(&self, height: Option<&str>) -> Result<R, E> {
        self.contract_call("queryAllAccountsNumber", &[], None, height)
    }
    ...
    E: Fail,
{
    ...
    /// Query the resource number of the permission
    ///
    /// return The resources number of permission
    fn query_resource_num(&self, address: &str, height: Option<&str>) -> Result<R, E> {
        self.contract_call_to_address("queryResourceNum", &[], address, height)
    }

    /// Check how hot about resource
    ///
    /// param contract: The contract address of the resource
    /// param func: The function signature of the resource
    /// return how hot about resource
    fn query_resource_hot(
        &self,
        address: &str,
        contract: &str,
        func: &str,
        ...
        let values = [remove_0x(contract), remove_0x(func)];
        self.contract_call_to_address("queryResourceHot", &values, address, height)
    }
}
    ...
    E: Fail,
{
    ...
    fn get_allowance_check(&self, height: Option<&str>) -> Result<R, E> {
        self.contract_call("getAllowanceCheck", &[], None, height)
    }
...
/// Certificate Revoke manager contract
#[derive(ContractExt)]
#[contract(addr = "0xffffffffffffffffffffffffffffffffff020030")]
#[contract(path = "../../contract_abi/CertRevokeManager.abi")]
#[contract(name = "CertRevokeManagerExt")]
pub struct CertRevokeManagerClient<T> {
    client: T,
    address: Address,
    contract: Contract,
}
...
/// Certificate Revoke  manager contract
pub trait CertRevokeManagerExt<T, R, E>: ContractCall<R, E>
where
    T: ClientExt<R, E>,
    R: serde::Serialize + serde::Deserialize<'static> + ::std::fmt::Display,
    E: Fail,
{
    ...
    fn revoke(&mut self, account_address: &str, quota: Option<u64>) -> Result<R, E> {
        let values = [remove_0x(account_address)];
        self.contract_send_tx("revoke", &values, quota, None)
    }

    /// release a certificate from revoke list using its common name.
    /// Actually the corresponding node address.
    fn release_revoke(&mut self, account_address: &str, quota: Option<u64>) -> Result<R, E> {
        let values = [remove_0x(account_address)];
        self.contract_send_tx("releaseRevoke", &values, quota, None)
    }
    ...
    fn set_committee(&mut self, committee_address: &str, quota: Option<u64>) -> Result<R, E> {
        let values = [remove_0x(committee_address)];
        self.contract_send_tx("setCommittee", &values, quota, None)
    }
}

/// Allowance Client
#[derive(ContractExt)]
#[contract(addr = "0xffffffffffffffffffffffffffffffffff020041")]
#[contract(path = "../../contract_abi/Allowance.abi")]
#[contract(name = "AllowanceExt")]
pub struct AllowanceClient<T> {
    client: T,
    address: Address,
    contract: Contract,
}
...
/// Allowance contract
pub trait AllowanceExt<T, R, E>: ContractCall<R, E>
where
    T: ClientExt<R, E>,
    R: serde::Serialize + serde::Deserialize<'static> + ::std::fmt::Display,
    E: Fail,
{
    ...
    /// Check resource in the allowance
    ///
    /// param door: The door address of the doornum
    /// return true if in allowance, otherwise false
    fn in_allowance(&self, door: &str, height: Option<&str>) -> Result<R, E> {
        let value = [remove_0x(door)];
        self.contract_call("inAllowance", &value, None, height)
    }

    /// Query the information of the allowance
    ///
    /// return The information of permission: name and resources
    fn query_info(&self, height: Option<&str>) -> Result<R, E> {
        self.contract_call("queryInfo", &[], None, height)
    }

    /// Query the name of the allowance
    ///
    /// return The name of allowance
    fn query_name(&self, height: Option<&str>) -> Result<R, E> {
        self.contract_call("queryName", &[], None, height)
    }

    /// Query the doornum of the allowance
    ///
    /// return The doornum of allowance
    fn query_doornum(&self, height: Option<&str>) -> Result<R, E> {
        self.contract_call("queryDoornum", &[], None, height)
    }

    /// Query the doornum length of the allowance
    ///
    /// return The doornum length of allowance
    fn query_doornum_length(&self, height: Option<&str>) -> Result<R, E> {
        self.contract_call("queryDoornumLength", &[], None, height)
    }

    /// Check how hot about door address
    ///
    /// param door: The door address of the doornum
    /// return how hot about door address
    fn query_doornum_hot(&self, door: &str, height: Option<&str>) -> Result<R, E> {
        let value = [remove_0x(door)];
        self.contract_call("queryDoorHot", &value, None, height)
    }
}

/// Allowance manage Client
#[derive(ContractExt)]
#[contract(addr = "0xffffffffffffffffffffffffffffffffff020040")]
#[contract(path = "../../contract_abi/AllowanceManagement.abi")]
#[contract(name = "AllowanceManagementExt")]
pub struct AllowanceManageClient<T> {
    client: T,
    address: Address,
    contract: Contract,
}
...
/// PermissionManagement system contract
pub trait AllowanceManagementExt<T, R, E>: ContractCall<R, E>
where
    T: ClientExt<R, E>,
    R: serde::Serialize + serde::Deserialize<'static> + ::std::fmt::Display,
    E: Fail,
{
    ...
    /// Update the allowance name
    ///
    /// param name: The new name
    /// return true if successed, otherwise false
    fn update_allowance_name(&mut self, name: &str, quota: Option<u64>) -> Result<R, E> {
        let value = [name];
        self.contract_send_tx("updateAllowanceName", &value, quota, None)
    }

    /// Add the doornums of allowance
    ///
    /// param doors: The doors of pre-to-added doornums
    /// param nums: The nums of pre-to-added doornums
    /// return true if successed, otherwise false
    fn add_doornums(&mut self, doors: &str, nums: &str, quota: Option<u64>) -> Result<R, E> {
        let values = [doors, nums];
        self.contract_send_tx("addDoornums", &values, quota, None)
    }

    /// update the doornum of allowance, once update one doornum
    ///
    /// param door: The door of pre-to-updated doornum
    /// param num: The num of pre-to-added doornum
    /// return true if successed, otherwise false
    fn update_doornum(&mut self, door: &str, num: &str, quota: Option<u64>) -> Result<R, E> {
        let values = [remove_0x(door), remove_0x(num)];
        self.contract_send_tx("updateDoornum", &values, quota, None)
    }

    /// Delete the resources of permission
    ///
    /// param doors: The doors of pre-to-deleted doornums
    /// return true if successed, otherwise false
    fn delete_doornums(&mut self, doors: &str, quota: Option<u64>) -> Result<R, E> {
        let value = [remove_0x(doors)];
        self.contract_send_tx("deleteDoornums", &value, quota, None)
    }
}
