...
impl fmt::Debug for PrivateKey {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let msg = match *self {
            PrivateKey::Secp256k1(private_key) => encode(private_key.0),
            PrivateKey::Sm2(private_key) => encode(private_key.0),
...
impl PubKey {
    ...
    pub fn to_vec(&self) -> Vec<u8> {
        match self {
            PubKey::Secp256k1(pk) | PubKey::Sm2(pk) => pk.0.to_vec(),
            PubKey::Null => Vec::new(),
        }
    }
}
...
impl fmt::Display for PubKey {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let msg = match *self {
            PubKey::Secp256k1(pubkey) => encode(pubkey.0),
            PubKey::Sm2(pubkey) => encode(pubkey.0),
...
impl fmt::Display for Signature {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match self {
            Signature::Secp256k1(sig) => write!(f, "{}", encode(&sig.0[..])),
            Signature::Sm2(sig) => write!(f, "{}", encode(&sig.0[..])),
