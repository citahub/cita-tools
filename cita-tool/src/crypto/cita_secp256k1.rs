...
impl CreateKey for Secp256k1KeyPair {
    ...
    fn gen_keypair() -> Self {
        let context = &SECP256K1;
        let (s, p) = context.generate_keypair(&mut OsRng::default());
...
impl Secp256k1Signature {
        ...
        self.v() <= 1
            && H256::from_slice(self.r())
                < H256::from_str("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141")
                    .unwrap()
            && H256::from_slice(self.r()) >= H256::from_low_u64_le(1)
            && H256::from_slice(self.s())
                < H256::from_str("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141")
                    .unwrap()
            && H256::from_slice(self.s()) >= H256::from_low_u64_le(1)
    }
    ...
    pub fn recover(&self, message: &Message) -> Result<Secp256k1PubKey, Error> {
        ...
        let public = context.recover_ecdsa(&SecpMessage::from_slice(&message.0[..])?, &rsig)?;
...
impl fmt::Debug for Secp256k1Signature {
    fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        f.debug_struct("Signature")
            .field("r", &encode(&self.0[0..32]))
            .field("s", &encode(&self.0[32..64]))
            .field("v", &encode(&self.0[64..65]))
            .finish()
    }
}
